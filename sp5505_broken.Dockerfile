...
RUN --mount=type=cache,target=/opt/buildcache,id=spack-binary-cache,sharing=locked \
    ...
    spack add \
        ...
        'cfitsio' \
        'curl' \
        'hdf5@'$HDF5_VERSION'+hl' \
        'mpich' \
        'openblas@:0.3.27' \
        'oskar@'$OSKAR_VERSION'~openmp~casacore' \
    ...
    export ac_cv_lib_curl_curl_easy_init=no && \
    spack install --no-check-signature --no-checksum --fail-fast python@3.10 py-pip py-numpy && \
...
RUN --mount=type=cache,target=/root/.cache/pip \
    ...
    echo "Locating Spack-installed OSKAR prefix..." && \
    OSKAR_PREFIX="$(spack -e none find --format '{prefix}' oskar@${OSKAR_VERSION}~openmp~casacore 2>/dev/null | tail -n1)" && \
    if [ -z "$OSKAR_PREFIX" ] || [ ! -d "$OSKAR_PREFIX" ]; then \
        echo "OSKAR prefix not found via spack; falling back to /opt/view"; \
        OSKAR_PREFIX="/opt/view"; \
    fi && \
    echo "OSKAR_PREFIX=$OSKAR_PREFIX" && \
    OSKAR_INC_DIR="$OSKAR_PREFIX/include" && \
    OSKAR_LIB_DIR="" && \
    if [ -d "$OSKAR_PREFIX/lib" ]; then OSKAR_LIB_DIR="$OSKAR_PREFIX/lib"; fi && \
    if [ -z "$OSKAR_LIB_DIR" ] && [ -d "$OSKAR_PREFIX/lib64" ]; then OSKAR_LIB_DIR="$OSKAR_PREFIX/lib64"; fi && \
    export OSKAR_INC_DIR OSKAR_LIB_DIR && \
    echo "Validating header: $OSKAR_INC_DIR/oskar/oskar_version.h" && \
    test -f "$OSKAR_INC_DIR/oskar/oskar_version.h" || { echo "ERROR: oskar_version.h not found under $OSKAR_INC_DIR"; ls -R "$OSKAR_PREFIX/include" || true; exit 1; } && \
    echo "Building OSKAR Python bindings with OSKAR_INC_DIR=$OSKAR_INC_DIR OSKAR_LIB_DIR=$OSKAR_LIB_DIR" && \
    export LD_LIBRARY_PATH="$OSKAR_LIB_DIR:/opt/view/lib:/opt/view/lib64:${LD_LIBRARY_PATH}" && \
    export CFLAGS="-I$OSKAR_INC_DIR ${CFLAGS}" && \
    export CXXFLAGS="-I$OSKAR_INC_DIR ${CXXFLAGS}" && \
