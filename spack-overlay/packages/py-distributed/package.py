# /opt/spack/var/spack/repos/builtin/packages/py-distributed/package.py
# docker build -f dask.Dockerfile --progress=plain -t d3vnull0/dask:latest /home/ubuntu/Karabo-Pipeline 2>&1 | tee dask.log
...
class PyDistributed(PythonPackage):
    ...
    version("2022.12.1", sha256="87fedcfbf2126d14c3865c8e445c41eccabd2b07eaff87528af2afd9e20ced6c")
    ...
    # Provide dask only for tests to avoid runtime circular dependencies
    ...
    def test(self):
        # Simple import/version test executed by `spack test run py-distributed`
        python = self.spec["python"].command
        python(
            "-c",
            "import distributed; print(getattr(distributed, '__version__', '0.0'))",
        )
