# /opt/spack/var/spack/repos/builtin/packages/py-dask/package.py
# https://github.com/spack/spack-packages/blob/develop/repos/spack_repo/builtin/packages/py_dask/package.py
...
class PyDask(PythonPackage):
    ...
    depends_on("py-distributed@2022.12.1", type=("build", "run"), when="@2022.12.1 +distributed")
    depends_on(
    ...
    def test(self):
        # Simple import/version test executed by `spack test run py-dask`
        python = self.spec["python"].command
        python(
            "-c",
            "import dask; print(getattr(dask, '__version__', '0.0'))",
        )
