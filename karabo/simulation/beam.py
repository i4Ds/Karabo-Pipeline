...
def generate_eidos_beam(npixels, image_width_degrees, frequencies,stokes='I'):
    
    """ This function generates a beam using EIDOS. Takes Image details as input and 
     saves the primary beams in same location as the code in which the funtion is called in
...
def generate_airy_beam_data(
    ...
    first_zero = fwhm_pixels / (2 * np.sqrt(2 * np.log(2)))  # approximate matching to Gaussian FWHM
...
def airy_beam_fwhm_for_frequency(
        frequency_hz: float, 
        dish_diameter_m: float) -> float:
    """
    Compute the Airy beam FWHM in degrees for a given observing frequency and dish diameter.
