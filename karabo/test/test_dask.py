...
def test_get_min_max_of_node_id(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_min_max_of_node_id() == (4397, 4406)


def test_get_lowest_node_id(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_lowest_node_id() == 4397


def test_get_base_string_node_list(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_base_string_node_list() == "nid0"


def test_get_lowest_node_name(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_lowest_node_name() == "nid04397"


def test_get_number_of_nodes(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_number_of_nodes() == 10


def test_get_node_id(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_node_id() == 4397


def test_get_node_name(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_node_name() == "nid04397"


def test_is_first_node(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.is_first_node() is True


def test_is_on_slurm_cluster(env_vars: _EnvVarsType) -> None:
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.is_on_slurm_cluster() is True
...
def test_multiple_nodes_and_ranges() -> None:
    ...
    with patch.dict(os.environ, env_vars):
        assert DaskSlurmHandler.get_min_max_of_node_id() == (2780, 4715)
        assert DaskSlurmHandler.get_lowest_node_id() == 2780
        assert DaskSlurmHandler.get_base_string_node_list() == "nid0"
        assert DaskSlurmHandler.get_lowest_node_name() == "nid02780"
        assert DaskSlurmHandler.get_number_of_nodes() == 3
        assert DaskSlurmHandler.get_node_id() == 2780
        assert DaskSlurmHandler.get_node_name() == "nid02780"
        assert DaskSlurmHandler.is_first_node() is True
        assert DaskSlurmHandler.is_on_slurm_cluster() is True
    ...
    with patch.dict(os.environ, env_vars):
        ...
        assert DaskSlurmHandler.get_lowest_node_name() == "nid02780"
        assert DaskSlurmHandler.get_number_of_nodes() == 3
        assert DaskSlurmHandler.get_node_id() == 4715
        assert DaskSlurmHandler.get_node_name() == "nid04715"
        assert DaskSlurmHandler.is_first_node() is False
        assert DaskSlurmHandler.is_on_slurm_cluster() is True
    ...
    with patch.dict(os.environ, env_vars):
        ...
        assert DaskSlurmHandler.get_lowest_node_name() == "nid02780"
        assert DaskSlurmHandler.get_number_of_nodes() == 3
        assert DaskSlurmHandler.get_node_id() == 2781
        assert DaskSlurmHandler.get_node_name() == "nid02781"
        assert DaskSlurmHandler.is_first_node() is False
        assert DaskSlurmHandler.is_on_slurm_cluster() is True
...
def test_extreme_range_of_nodes() -> None:
    ...
    with patch.dict(os.environ, env_vars):
        ...
        assert DaskSlurmHandler.get_base_string_node_list() == "nid0"
        assert DaskSlurmHandler.get_lowest_node_name() == "nid02780"
        assert DaskSlurmHandler.get_number_of_nodes() == 1106
        assert DaskSlurmHandler.get_node_id() == 3333
        assert DaskSlurmHandler.get_node_name() == "nid03333"
        assert DaskSlurmHandler.is_first_node() is False
        assert DaskSlurmHandler.is_on_slurm_cluster() is True
        assert len(DaskSlurmHandler.extract_node_ids_from_node_list()) == 1106
...
def test_single_node() -> None:
    ...
    with patch.dict(os.environ, env_vars):
        min_node_id, max_node_id = DaskSlurmHandler.get_min_max_of_node_id()
        assert min_node_id == 3038
        assert max_node_id == 3038
        assert DaskSlurmHandler.get_base_string_node_list() == "nid"
