# docker build -f dask.Dockerfile --progress=plain -t d3vnull0/dask:latest . 2>&1 | tee dask.log
...
RUN --mount=type=cache,target=/opt/buildcache,id=spack-binary-cache,sharing=locked \
    ...
    ac_cv_lib_curl_curl_easy_init=no spack --verbose install --no-check-signature --no-checksum --fail-fast --show-log-on-error && \
    spack env view regenerate && \
    spack test run 'py-dask' && \
    # Validate distributed import using Spack view Python with dask present
    /opt/view/bin/python -c "import dask, distributed; print('OK', dask.__version__)"
